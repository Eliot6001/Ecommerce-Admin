...
export const CellAction: React.FC<CellActionProps> = ({ data }) => {
  ...
  const onCopy = (id: string) => {
    ...
    toast.success("Size id added to clipboard");
  };
  const forwardEditSize = (id: string) => {
  ...
  const onDelete = async (id: string) => {
    try {
      setLoading(true);
      await axios.delete(
        `/api/${params.storeId}/sizes/${id}`
      );
      router.refresh();
      toast.success("Size Successfully Deleted!");
    } catch (error) {
      toast.error("Make sure you've removed every product inside the Sizes");
      console.log(error)
    } finally {
      setLoading(false);
      setOpen(false);
    ...
    <>
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end">
          ...
          <DropdownMenuSeparator />
          <DropdownMenuItem onClick={() => forwardEditSize(data.id)}>
            <Edit className="w-4 h-4 mr-1" /> Edit Size
          </DropdownMenuItem>
          ...
          <DropdownMenuItem className="bg-red-500 focus:bg-red-600/95 text-white focus:text-slate-100" onClick={() => setOpen(true)}>
            <Trash className="w-4 h-4 mr-1 " /> Delete Size
          </DropdownMenuItem>
        </DropdownMenuContent>
      </DropdownMenu>
    </>
